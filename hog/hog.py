...
def roll_dice(num_rolls, dice=six_sided):
    ...
    "*** YOUR CODE HERE ***"
    # END PROBLEM 1
    # 思路为循环num_rolls次，如果dice函数返回结果为1那么就跳出循环返回1
    flag = 0
    sum = 0
...
def boar_brawl(player_score, opponent_score):
    ...
    # END PROBLEM 2
    temp = 0
    opponent_tens_score = 0
    if opponent_score >= 10:
        temp = opponent_score // 10
        opponent_tens_score = temp % 10
    player_ons_score = player_score % 10
    return max(1,3*abs(opponent_tens_score - player_ons_score))
...
def take_turn(num_rolls, player_score, opponent_score, dice=six_sided):
    ...
    # END PROBLEM 3
    # 有两种情况
    # 第一种抽到了0 那么调用boar_brawl函数统计分数
    # 第二种情况没抽到0 持续计算总值，返回。除非抽到1
...
def sus_points(score):
    ...
    "*** YOUR CODE HERE ***"
    # END PROBLEM 4
    if num_factors(score) > 2 and num_factors(score) < 5:
        # 这里遇到一个问题，如何找一个数最近的素数，如果每次加一在循环时间复杂度为O(n方)
        while 1:
            score += 1
            if is_prime(score):
                return score 
...
def play(strategy0, strategy1, update,
    ...
    while score0 < goal and score1 <goal:  
        if who == 0:
            score0 = update(strategy0(score1,score1),score0,score1,dice)
        else:
